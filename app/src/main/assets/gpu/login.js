...
(function() {
    ...
    window.attemptLogin = function() {
        ...
        if (username === "" || password === "") {
            alert("عذراً، يرجى ملء الخانات أولاً للدخول!");
            return;
        }
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                if (data.includes("ok")) {
                    // [ميزة الحفظ]: تخزين البيانات بنجاح لعدم تكرار كتابتها
                    ...
                    // إذا فشل الدخول التلقائي لسبب ما (تغيير كلمة المرور مثلاً)، نحذف المخزن القديم
    ...
    function checkSavedPlayer() {
        ...
        // إذا وُجدت مفاتيح مخزنة سابقاً، يتم حقنها وإطلاق طلب الدخول فوراً وبشكل تلقائي
        if (savedUser && savedPass) {
            document.getElementById('user-name-field').value = savedUser;
            document.getElementById('user-pass-field').value = savedPass;
            
            console.log("تم العثور على لاعب محفوظ.. جاري تسجيل الدخول التلقائي البيني..");
            window.attemptLogin();
        }
    }

    // تشغيل الفحص التلقائي بعد حقن نافذة الـ HTML مباشرة بمهلة بسيطة لضمان استقرار العناصر
